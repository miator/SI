...
from src.config.data_config import NOISE_TRAIN_FILES_FRACTION, NOISE_SPLIT_SEED


def scan_noise_files(
    noise_root,
    sample_rate: int,
    min_noise_seconds: float,
) -> list[Path]:
...
def split_noise_paths(
    ...
    train_fraction: float = NOISE_TRAIN_FILES_FRACTION,
    seed: int = NOISE_SPLIT_SEED,
    ...
    shuffled = list(noise_paths)  # shallow copy
